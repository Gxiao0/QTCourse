...
bool ServerWorker::setSocketDescriptor(qintptr socketDescriptor)//外部调用该函数来关联ServerWorker和具体的客户端连接套接字
{
...
void ServerWorker::onReadyRead()//读取客户端发送的数据
{
    ...
    for(;;){
        ...
        if(socketStream.commitTransaction()){//事务提交成功
            // emit logMessage(QString::fromUtf8(jsonData));//在对应的文本框中输出读取到的数据
